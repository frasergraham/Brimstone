...
Phase 6 layers atmosphere on top of the functional 3D scene: time-of-day lighting, a fog-of-war veil, glowing Power Nodes, and an animated selection halo. Lighting is keyed off `state.phase` through the exported `PHASE_LIGHT_CONFIG` table — warm amber at dawn (intensity 0.90), white-ish at day (1.10), orange-red at dusk (0.85), cool blue at night (0.55) — and the hemispheric light's intensity + diffuse colour plus the scene's clear colour all cross-fade over 3 s when the phase advances; interpolation runs each frame from `Scene.onBeforeRenderObservable` and uses `easeInOutCubic` so transitions read as smooth, not stepped. Each Power Node hex grows a thin upward emissive shaft and a flat emissive disc, tinted gold/green/pale-white/amber by `getNodeGlowColor(controller)` (hero/witch/neutral/contested), pulsing on a 3 s sine via `pulseFactor`; the shafts opt into a shared Babylon `GlowLayer` so they bloom outward from the playmat. The selection halo uses the same GlowLayer with a tighter 1.5 s pulse on the cyan base disc, replacing the flat-cyan swap Phase 3 originally shipped. Fog of war is gated on the same `state.fogOfWar !== 'none'` flag as the 2D path: `_buildFogVisibleHexes(observerOwner)` returns the union of all sight discs around the observer's alive entities (real implementation now lives behind the pure `buildFogVisibleSet` helper, sharing `sightRangeForEntity` with the 2D renderer); tiles outside the visible set get their material swapped to a darker variant (`FOG_TILE_DARKEN ≈ 0.32` multiplier), props (forest cones, road decks, bridge planks, buildings/roofs) hide via `isVisible = false`, and standees in fogged hexes hide the same way without disposing — so reveals re-enable them instantly. AI-vs-AI and spectator modes resolve the observer to `null` and suppress the veil entirely.

**Night lanterns.** A self-contained Phase 6 sub-system layers a per-unit `BABYLON.PointLight` on top of the hemispheric light to read as carried lanterns after sunset. Each living entity gets one warm amber light (`LANTERN_COLOR_HEX = #ffb060`) parented to its standee base at `LANTERN_HEIGHT_OFFSET` chest height, range `LANTERN_RANGE = 3.5` world units (≈3–4 hex radii). Peak intensity per phase comes from `lanternIntensityForPhase(phase)` — night 0.8, dusk 0.4, dawn 0.3, day 0 — and cross-fades over `LANTERN_FADE_MS` (matched to `PHASE_TRANSITION_MS` so the hemispheric and lantern transitions land in lockstep). Per-frame, `_pumpLanternFlicker(now)` multiplies the eased base by `flickerScale(now, phaseOffset)` — a two-band oscillator (slow ~2 Hz breathing, fast ~7 Hz pseudo-noise) clamped to [0.7, 1.1] — and each lantern carries its own random `phaseOffset` so a cluster of standees doesn't strobe in sync. Lifecycle is diff-driven via the pure `diffLanternLifecycle(priorIds, entities)` helper: live entities get a fresh `PointLight`, dead/missing ones dispose, fogged-out standees disable (`light.setEnabled(false)`) without dispose so re-reveals snap straight back. Babylon's default 4-light per-material cap is bumped to `LANTERN_MATERIAL_LIGHT_CAP = 16` on every existing `StandardMaterial` the first time the subsystem fires (plus any new materials minted later via `scene.onNewMaterialAddedObservable`), so the 10–20 active lanterns on a Standard map don't get silently dropped past slot 4. The whole subsystem is intentionally additive and stays clear of `_glowLayer.intensity`, `SELECTION_*`, and `FOG_TILE_DARKEN` so it composes cleanly with the rest of the atmosphere stack.
