...
- **MARCH** — every leader move site routes through `queueLeaderStep()`: when the leader's faction `canMarch()` and the march would actually move ≥2 nearby soldiers, it emits ONE `MARCH` that advances the whole formation (node push, retreat, shelter, explore escort, hunt). Who moves is decided by the shared `computeMarchPlacements()` — every friendly mobile soldier **within 1 hex** shifts by the captain's move vector (formation shift), converges toward the destination if that hex is blocked, or holds; capacity is honoured so a full destination simply yields fewer movers and falls back to a plain MOVE. `queueLeaderStep()` dry-runs that placement to count movers, and `HeroEnginePlanSimState.applyMarch()` projects the passengers (same function) so later-generated actions see correct positions; carried soldiers are committed for the round. `_promoteToMarchLeader()` upgrades a node-duty pick that landed on a co-located stack soldier to the captain himself. `assemblePlan` treats `MARCH` like `MOVE` for anti-oscillation and dedups it by destination (a chained march is two distinct steps).
