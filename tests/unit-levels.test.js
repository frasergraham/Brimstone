...
describe('level scaling helpers (Standard curve)', () => {
  test('hpForLevel = base × (1 + 0.5·(L−1)), rounded', () => {
    assert.equal(LEVEL_HP_PER, 0.5);
    assert.equal(hpForLevel(14, 1), 14);
    assert.equal(hpForLevel(14, 2), 21);
    assert.equal(hpForLevel(14, 3), 28);
    assert.equal(hpForLevel(21, 3), 42); // wood golem base 21 → L3
  });
...
describe('applyLevel + entity composition', () => {
  ...
  test('applyLevel scales HP and exposes the ATK/DEF bonus via getters', () => {
    ...
    assert.equal(z.maxHp, 28);          // 14 × 2.0
    assert.equal(z.hp, 28, 'spawns at full HP');
  ...
  test('level bonus composes ON TOP of base stats for survivors (per-character base)', () => {
    ...
    applyLevel(s, 2);
    assert.equal(s.maxHp, Math.round(baseHp * 1.5));
