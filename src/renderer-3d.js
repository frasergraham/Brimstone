...
// Importing the module registers .glb / .gltf plugins on BABYLON.SceneLoader
// as a side-effect. Lazy-imported by `_loadPaladinModel` so the loader bundle
// only ships to clients that actually render the 3D scene.
...
export class Renderer3D {
  ...
  async loadImages(basePath = 'assets') {
    // Remember basePath so `_loadPaladinModel` (kicked off from `_initBabylon`
    // once the scene exists) can fetch the GLB from the same root the
    // tilemap came from.
  ...
  _buildStandeeMesh(entity) {
    ...
    const standee = { plane: cone, base, sphere, leader, paladinClone: null };
